class Bech32:
    """
    Algorithm implementation for checking segwit address validity
    ...
    def bech32_decode_check(self, bech):
        hrp, data = self.bech32_decode(bech)
        if not hrp or not data:
            raise ValueError('{bech} is not a valid segwit address.'.format(bech))
